...
(function() {
  ...
  // Create/Update chart with lazy rendering (only when tab visible)
  function mkChart(canvasId, type, labels, datasets, title) {
    var canvas = document.getElementById(canvasId);
    if (!canvas) return;

    // Lazy render - skip if chart not visible
  ...
  function renderReasons() {
    ...
    d.forEach(function(r) {
      var k = (r.reason || 'Unknown') + '|' + (r.sla_code || 'Unknown');
      rsla[k] = (rsla[k] || 0) + 1;
    });
  ...
  function renderApps() {
    ...
    var asla = {};
    d.forEach(function(r) {
      var k = (r.application || 'Unknown') + '|' + (r.sla_code || 'Unknown');
      asla[k] = (asla[k] || 0) + 1;
    });
  ...
  function renderAOS() {
    ...
    var aosData = d.filter(function(r) { return r.aos && r.aos !== '0'; });
    var total = d.length;
    ...
    if (kp) {
      kp.innerHTML = [
        ...
        { l: 'Agents', v: uniqueCount(aosData, 'agent'), s: 'unique' },
      ].map(function(k) {
  ...
  function renderPending() {
    ...
    Object.keys(rendered).forEach(function(id) {
      var r = rendered[id];
      if (r) {
        mkChart(r.canvasId, r.type, r.labels, r.datasets);
      }
    });
  }
